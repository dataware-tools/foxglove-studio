...
import { Annotation, AnnotationForImageOnTimePoint, InterpolatedAnnotation, Point } from "../types";

const toZeroIfNaN = (value: number) => (!isNaN(value) && isFinite(value) ? value : 0);
...
const genLinearInterpolator =
  (sortedAnnotations: AnnotationForImageOnTimePoint[]) => (currentTimeInNumber: number) => {
    const startCandidates = sortedAnnotations.filter((a) => a.timestamp_to < currentTimeInNumber);
    const start = startCandidates[startCandidates.length - 1];
    const end = sortedAnnotations.find((a) => a.timestamp_from >= currentTimeInNumber);
    ...
    const interpolationCoefficient =
      (currentTimeInNumber - start.timestamp_to) / (end.timestamp_from - start.timestamp_to);
...
const checkIfAnnotationCanBeInterpolated = (annotation: Annotation): boolean => {
...
export const useInterpolatedAnnotations = (): {
  ...
  const interpolatedAnnotations = useMemo(() => {
    ...
    const annotationsShouldBeInterpolated: Record<string, AnnotationForImageOnTimePoint[]> = {};
    ...
    annotations.forEach((annotation) => {
      ...
      const annotationsMayBeInterPolated = annotations.filter(
        (a) => checkIfAnnotationCanBeInterpolated(a) && a.instance_id === instanceId,
      );
    ...
    Object.entries(annotationsShouldBeInterpolated).forEach(([instanceId, annotations]) => {
      const sortedAnnotations = annotations.sort((a, b) => a.timestamp_from - b.timestamp_from);
      const annotationFrom = sortedAnnotations[0];
      const annotationTo = sortedAnnotations[sortedAnnotations.length - 1];
      if (!annotationFrom || !annotationTo) return;

      const interpolatedAnnotation: InterpolatedAnnotation = {
        index: 0,
        id: `interpolated-${annotationFrom.id}`,
        timestamp_from: annotationFrom.timestamp_from,
        timestamp_to: annotationTo.timestamp_to,
        type: "interpolate",
        comment: annotationFrom.comment,
        instance_id: instanceId,
        // TODO: targetTopic and frame_id should be changed depends on timestamp
        targetTopic: annotationFrom.targetTopic || "",
        frame_id: annotationFrom.frame_id || "",
